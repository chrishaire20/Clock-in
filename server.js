...
app.post('/api/send-report', requireAdmin, async (req, res) => {
    ...
    // Fetch punches and tips for the date range
    const getPunches = () => new Promise((resolve, reject) => {
        db.all(
            `SELECT p.*, e.name as employee_name, e.hourly_rate
             FROM punches p JOIN employees e ON p.employee_id = e.id
             WHERE date(p.timestamp) BETWEEN ? AND ?
             ORDER BY e.name, p.timestamp`,
            [startDate, endDate],
    ...
    const getTips = () => new Promise((resolve, reject) => {
        db.all(
            `SELECT t.*, e.name as employee_name
             FROM tips t JOIN employees e ON t.employee_id = e.id
             WHERE t.date BETWEEN ? AND ?
             ORDER BY e.name, t.date`,
            [startDate, endDate],
            (err, rows) => err ? reject(err) : resolve(rows || [])
    ...
    try {
        ...
        const empData = {};
        punches.forEach(p => {
            if (!empData[p.employee_name]) {
                empData[p.employee_name] = { punches: [], hourly_rate: p.hourly_rate, totalMinutes: 0, lastIn: null };
            }
            const emp = empData[p.employee_name];
            emp.punches.push(p);
            const ts = new Date(p.timestamp);
            if (p.punch_type === 'in') emp.lastIn = ts;
            else if ((p.punch_type === 'out' || p.punch_type === 'break_start') && emp.lastIn) {
                emp.totalMinutes += (ts - emp.lastIn) / 60000;
                emp.lastIn = p.punch_type === 'break_start' ? null : null;
            } else if (p.punch_type === 'break_end') {
                emp.lastIn = ts;
            }
        });
        ...
        const tipsByEmp = {};
        tips.forEach(t => {
            if (!tipsByEmp[t.employee_name]) tipsByEmp[t.employee_name] = { cash: 0, credit: 0 };
            tipsByEmp[t.employee_name].cash += t.cash_tips || 0;
            tipsByEmp[t.employee_name].credit += t.credit_tips || 0;
        });

        // Build plain-text report
        let reportText = `RESTAURANT TIME CLOCK REPORT\n`;
        reportText += `Period: ${startDate} to ${endDate}\n`;
        reportText += `Generated: ${new Date().toLocaleString()}\n`;
        reportText += `${'='.repeat(50)}\n\n`;

        let grandHours = 0, grandPay = 0, grandTips = 0;

        Object.entries(empData).forEach(([name, data]) => {
            const hours = data.totalMinutes / 60;
            const pay = hours * data.hourly_rate;
            const empTips = tipsByEmp[name] || { cash: 0, credit: 0 };
            const totalTips = empTips.cash + empTips.credit;
            grandHours += hours;
            grandPay += pay;
            grandTips += totalTips;

            reportText += `${name}\n`;
            reportText += `  Hours: ${hours.toFixed(2)} @ $${data.hourly_rate}/hr = $${pay.toFixed(2)}\n`;
            reportText += `  Tips: Cash $${empTips.cash.toFixed(2)} | Credit $${empTips.credit.toFixed(2)} | Total $${totalTips.toFixed(2)}\n\n`;
        });

        reportText += `${'='.repeat(50)}\n`;
        reportText += `TOTALS\n`;
        reportText += `  Total Hours: ${grandHours.toFixed(2)}\n`;
        reportText += `  Total Gross Pay: $${grandPay.toFixed(2)}\n`;
        reportText += `  Total Tips: $${grandTips.toFixed(2)}\n`;
        ...
        if (REPORT_EMAIL && EMAIL_USER && EMAIL_PASS) {
            ...
            await transporter.sendMail({
                from: EMAIL_USER,
                to: REPORT_EMAIL,
                subject: `Time Clock Report: ${startDate} to ${endDate}`,
                text: reportText
            });

            res.json({ success: true, message: `Report emailed to ${REPORT_EMAIL}` });
        } else {
            // Email not configured — return the report text so frontend can show/download it
            res.json({ success: true, emailFailed: true, report: reportText, message: 'Email not configured — use Print instead' });
        }
    } catch (err) {
