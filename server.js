...
async function runDailyDriveBackup() {
    ...
    try {
        ...
        const todayPunches = await query(
            `SELECT p.*, e.name as employee_name, e.hourly_rate FROM punches p JOIN employees e ON p.employee_id=e.id WHERE DATE(p.timestamp)=? ORDER BY e.name, p.timestamp`,
            [today]
        );
...
app.get('/api/admin/employee/:id', requireAdmin, async (req, res) => {
    ...
    try {
        ...
        const punchQ = start && end
            ? `SELECT * FROM punches WHERE employee_id=? ${dateFilter('timestamp', start, end)} ORDER BY timestamp ASC`
            : `SELECT * FROM punches WHERE employee_id=? ORDER BY timestamp ASC`;
        const tipQ = start && end
            ? `SELECT * FROM tips WHERE employee_id=? AND date BETWEEN ? AND ? ORDER BY date ASC`
            : `SELECT * FROM tips WHERE employee_id=? ORDER BY date ASC`;

        const punches = await query(punchQ, start && end ? [id, start, end] : [id]);
        const tips = start && end
            ? await query(tipQ, [id, start, end])
            : await query(tipQ, [id]);
