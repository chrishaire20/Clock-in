...
// ── PIN CACHE — stores last successful match for ~60s to make repeat logins instant ──
const pinCache = new Map(); // pin -> { empId, expires }

app.post('/api/login', async (req, res) => {
    const { pin } = req.body;
    if (!pin) return res.status(400).json({ success: false, message: 'PIN required' });
    try {
        // Check cache first for instant response on repeat login
        const now = Date.now();
        const cached = pinCache.get(pin);
        if (cached && cached.expires > now) {
            const employees = await query('SELECT * FROM employees WHERE id=? AND active=1', [cached.empId]);
            if (employees.length) {
                const emp = employees[0];
                req.session.employeeId = emp.id;
                req.session.employeeName = emp.name;
                req.session.role = emp.role;
                return res.json({ success: true, employee: { id: emp.id, name: emp.name, role: emp.role } });
            }
        }
        const employees = await query('SELECT * FROM employees WHERE active=1');
        // Check all employees in parallel instead of sequential sync blocking
        const results = await Promise.all(
            employees.map(e => bcrypt.compare(pin, e.pin).then(match => match ? e : null))
        );
        const emp = results.find(e => e !== null);
        if (emp) {
            // Cache the match for 60 seconds
            pinCache.set(pin, { empId: emp.id, expires: now + 60000 });
            req.session.employeeId = emp.id;
            req.session.employeeName = emp.name;
            req.session.role = emp.role;
...
app.post('/api/admin/employees', requireAdmin, async (req, res) => {
    ...
    try {
        const rows = await query(
            ...
            [name, bcrypt.hashSync(pin, 8), parseFloat(hourly_rate) || 15.00, role || 'employee']
        );
...
app.put('/api/admin/employees/:id', requireAdmin, async (req, res) => {
    ...
    try {
        if (pin && pin.length === 4) {
            // Clear any cached PIN for this employee since it's changing
            for (const [cp, val] of pinCache.entries()) {
                if (val.empId == req.params.id) pinCache.delete(cp);
            }
            await query('UPDATE employees SET name=?,hourly_rate=?,active=?,pin=?,pay_period=? WHERE id=?',
                [name, parseFloat(hourly_rate), active, bcrypt.hashSync(pin, 8), pp, req.params.id]);
        } else {
