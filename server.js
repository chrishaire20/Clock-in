...
async function initPostgres() {
    ...
    // Add columns to punches if missing
    await db.query(`ALTER TABLE punches ADD COLUMN IF NOT EXISTS work_role TEXT`).catch(() => {});
    await db.query(`ALTER TABLE punches ADD COLUMN IF NOT EXISTS role_rate REAL`).catch(() => {});
    await db.query(`ALTER TABLE punches ADD COLUMN IF NOT EXISTS note TEXT`).catch(() => {});
    // Patch employee_roles columns if table already existed without them
    await db.query(`ALTER TABLE employee_roles ADD COLUMN IF NOT EXISTS role_name TEXT NOT NULL DEFAULT ''`).catch(() => {});
    await db.query(`ALTER TABLE employee_roles ADD COLUMN IF NOT EXISTS employee_id INTEGER`).catch(() => {});
    await db.query(`ALTER TABLE employee_roles ADD COLUMN IF NOT EXISTS hourly_rate REAL DEFAULT 15.00`).catch(() => {});
    // Add can_log_hours if missing
    await db.query(`ALTER TABLE employees ADD COLUMN IF NOT EXISTS can_log_hours INTEGER DEFAULT 1`).catch(() => {});
    await seedEmployees();
...
async function initSQLite() {
    ...
    await query(`ALTER TABLE punches ADD COLUMN role_rate REAL`).catch(() => {});
    // Add hourly_rate to employee_roles if missing
    await query(`ALTER TABLE employee_roles ADD COLUMN hourly_rate REAL DEFAULT 15.00`).catch(() => {});
    // Add can_log_hours if missing
    await query(`ALTER TABLE employees ADD COLUMN can_log_hours INTEGER DEFAULT 1`).catch(() => {});
    await seedEmployees();
    await seedEmployeeRoles();
}
...
async function seedEmployees() {
    ...
    for (const [name, pin, rate, role] of employees) {
        const hashed = bcrypt.hashSync(pin, 4);
...
async function seedEmployeeRoles() {
    // Default roles with pay rates per employee name
    // This runs every startup and only inserts roles for employees who have NONE yet
    const defaultRoles = {
        'Marline':     [['Prep', 15.00], ['Cook', 17.00]],
        'Marlene':     [['Prep', 15.00], ['Cook', 17.00]],
        'Kara':        [['Prep', 15.00], ['Dish', 14.00], ['Serving', 15.00]],
        'Christopher': [['Serving', 15.00], ['Cook', 17.00], ['Prep', 15.00], ['Dish', 14.00]],
        'Sarah':       [['Serving', 15.00], ['Dish', 14.00]],
        'Nathan':      [['Serving', 15.00], ['Cook', 17.00], ['Prep', 15.00], ['Dish', 14.00]],
        'Pam':         [['Serving', 15.00], ['Cook', 17.00], ['Prep', 15.00], ['Dish', 14.00]],
        'Aaron':       [['Prep', 15.00], ['Cook', 17.00], ['Dish', 14.00]],
        'Gigi':        [['Serving', 15.00], ['Prep', 15.00], ['Dish', 14.00]],
        'Natalie':     [['Serving', 15.00], ['Prep', 15.00], ['Dish', 14.00]],
    };
    const emps = await query("SELECT id, name FROM employees WHERE role='employee'");
    for (const emp of emps) {
        // Check if this specific employee already has roles
        const existing = await query('SELECT COUNT(*) as count FROM employee_roles WHERE employee_id=?', [emp.id]);
        const count = parseInt(existing[0].count || existing[0]['COUNT(*)'] || 0);
        if (count > 0) continue; // already set up, skip
        const roles = defaultRoles[emp.name] || [['Server', 15.00], ['Cook', 17.00], ['Prep', 15.00], ['Dish', 14.00]];
        for (const [roleName, rate] of roles) {
            await query('INSERT INTO employee_roles (employee_id, role_name, hourly_rate) VALUES (?,?,?)', [emp.id, roleName, rate]);
        }
        console.log(`✅ Seeded roles for ${emp.name}`);
    }
}
...
// calcGrossPay — uses role_rate stored on each clock-in punch when available,
// falls back to the employee's base hourly_rate otherwise.
function calcGrossPay(punches, baseRate) {
    let gross = 0, lastIn = null, lastRate = baseRate;
    [...punches].sort((a, b) => new Date(a.timestamp) - new Date(b.timestamp)).forEach(p => {
        if (p.punch_type === 'in') {
            lastIn = new Date(p.timestamp);
            lastRate = (p.role_rate != null) ? parseFloat(p.role_rate) : baseRate;
        } else if (p.punch_type === 'out' && lastIn) {
            const mins = (new Date(p.timestamp) - lastIn) / 60000;
            gross += (mins / 60) * lastRate;
            lastIn = null;
        }
    });
    return gross;
...
async function buildReportText(start, end) {
    ...
    Object.entries(empData).forEach(([name, data]) => {
        ...
        const p = calcGrossPay(data.punches, data.hourly_rate);
        const t = tipsByEmp[name] || 0;
        grandH += h; grandP += p; grandT += t;
        text += `${name}\n  Hours: ${h.toFixed(2)} = $${p.toFixed(2)} (role-based rates)\n  Tips: $${t.toFixed(2)}\n  Total Earnings: $${(p + t).toFixed(2)}\n\n`;
    });
...
// ── FAST PIN MAP — built at startup, updated on changes ──
// PINs are 4 digits. We use bcrypt cost 4 (~5ms) instead of cost 10 (~300ms).
// On startup we re-hash any existing cost-10 hashes to cost 4.
// The pinMap gives O(1) instant login: pin -> employee object
const pinMap = new Map(); // hashed_pin_cost4 -> employee  (rebuilt on startup + updates)
let employeeCache = [];   // full employee list, refreshed on any change
...
async function buildPinMap() {
    try {
        ...
        for (const emp of employees) {
            pinMap.set(emp.id, emp);
        }
        console.log(`✅ PIN map built for ${employees.length} employees`);
    } catch (err) { console.error('PIN map build failed:', err.message); }
}

async function warmupPinMap() {
    // Re-hash all existing employees from cost-10 to cost-4 for fast login
    try {
        const employees = await query('SELECT * FROM employees');
        let migrated = 0;
        for (const emp of employees) {
            // Detect cost by reading the hash rounds — $2b$10$ vs $2b$04$
            if (emp.pin && emp.pin.includes('$2b$10$')) {
                // We can't reverse bcrypt, but we know the original PINs are 4 digits
                // Try all 10000 combinations to find the match, re-hash at cost 4
                for (let p = 0; p <= 9999; p++) {
                    const candidate = String(p).padStart(4, '0');
                    const match = await bcrypt.compare(candidate, emp.pin);
                    if (match) {
                        const newHash = await bcrypt.hash(candidate, 4);
                        await query('UPDATE employees SET pin=? WHERE id=?', [newHash, emp.id]);
                        migrated++;
                        console.log(`✅ Migrated PIN for ${emp.name}`);
                        break;
                    }
                }
            }
        }
        if (migrated > 0) console.log(`✅ Migrated ${migrated} PINs to cost-4 for fast login`);
        await buildPinMap();
    } catch (err) { console.error('PIN warmup failed:', err.message); }
}
...
app.post('/api/login', async (req, res) => {
    ...
    try {
        // Fast path: check against all active employees using cost-4 bcrypt (~5ms each)
        const employees = employeeCache.length ? employeeCache : await query('SELECT * FROM employees WHERE active=1');
        const results = await Promise.all(
            employees.filter(e => e.active != 0).map(e =>
                bcrypt.compare(pin, e.pin).then(match => match ? e : null)
            )
        );
        const emp = results.find(e => e !== null);
        if (emp) {
...
app.get('/api/status', async (req, res) => {
    ...
    try {
        ...
        const empRows = await query('SELECT can_log_hours FROM employees WHERE id=?', [req.session.employeeId]);
        const canLogHours = empRows.length ? (empRows[0].can_log_hours != 0) : true;
        res.json({ success: true, employeeName: req.session.employeeName, role: req.session.role, lastPunch: rows[0] || null, canLogHours });
    } catch (err) { res.status(500).json({ success: false, error: err.message }); }
});
...
app.post('/api/clock', async (req, res) => {
    ...
    try {
        await query('INSERT INTO punches (employee_id,punch_type,work_role,role_rate) VALUES (?,?,?,?)',
            [req.session.employeeId, type, work_role || null, role_rate ? parseFloat(role_rate) : null]);
        res.json({ success: true, type, timestamp: new Date().toISOString() });
    } catch (err) { res.status(500).json({ success: false, error: err.message }); }
});

// Employee self-submit: log a missed shift (creates in + out punch pair)
app.post('/api/clock/manual', async (req, res) => {
    if (!req.session.employeeId) return res.status(401).json({ success: false });
    // Check if employee is allowed to log hours
    try {
        const empRows = await query('SELECT can_log_hours FROM employees WHERE id=?', [req.session.employeeId]);
        if (empRows.length && empRows[0].can_log_hours == 0) {
            return res.status(403).json({ success: false, message: 'Hour logging is not enabled for your account. Please see a manager.' });
        }
    } catch (err) { /* allow through if check fails */ }
    const { clock_in, clock_out, work_role, role_rate, note } = req.body;
    if (!clock_in || !clock_out) return res.status(400).json({ success: false, message: 'clock_in and clock_out required' });
    const inTime = new Date(clock_in);
    const outTime = new Date(clock_out);
    if (isNaN(inTime) || isNaN(outTime)) return res.status(400).json({ success: false, message: 'Invalid date/time' });
    if (outTime <= inTime) return res.status(400).json({ success: false, message: 'Clock-out must be after clock-in' });
    const maxHours = 24;
    if ((outTime - inTime) / 3600000 > maxHours) return res.status(400).json({ success: false, message: 'Shift cannot exceed 24 hours' });
    try {
        await query('INSERT INTO punches (employee_id, punch_type, timestamp, work_role, role_rate, note) VALUES (?,?,?,?,?,?)',
            [req.session.employeeId, 'in', inTime.toISOString(), work_role || null, role_rate ? parseFloat(role_rate) : null, note || 'Manual entry']);
        await query('INSERT INTO punches (employee_id, punch_type, timestamp, work_role, role_rate, note) VALUES (?,?,?,?,?,?)',
            [req.session.employeeId, 'out', outTime.toISOString(), work_role || null, role_rate ? parseFloat(role_rate) : null, note || 'Manual entry']);
...
app.get('/api/my-roles', async (req, res) => {
    if (!req.session.employeeId) return res.status(401).json({ success: false });
    try {
        const rows = await query('SELECT id, role_name, hourly_rate FROM employee_roles WHERE employee_id=? ORDER BY id', [req.session.employeeId]);
...
app.get('/api/admin/employee/:id/roles', requireAdmin, async (req, res) => {
    try {
        const rows = await query('SELECT id, role_name, hourly_rate FROM employee_roles WHERE employee_id=? ORDER BY id', [req.params.id]);
        res.json({ success: true, roles: rows });
    } catch (err) { res.status(500).json({ success: false, error: err.message }); }
});
...
app.post('/api/admin/employee/:id/roles', requireAdmin, async (req, res) => {
    ...
    try {
        const result = await query('INSERT INTO employee_roles (employee_id, role_name, hourly_rate) VALUES (?,?,?)',
            [req.params.id, role_name.trim(), parseFloat(hourly_rate) || 15.00]);
...
app.put('/api/admin/employee/roles/:roleId', requireAdmin, async (req, res) => {
    ...
    if (!role_name || !role_name.trim()) return res.status(400).json({ success: false, message: 'Role name required' });
    try {
        await query('UPDATE employee_roles SET role_name=?, hourly_rate=? WHERE id=?',
            [role_name.trim(), parseFloat(hourly_rate) || 15.00, req.params.roleId]);
...
app.get('/api/admin/employees', requireAdmin, async (req, res) => {
    try {
        const rows = await query('SELECT id,name,role,hourly_rate,pay_period,active,created_at,can_log_hours FROM employees ORDER BY name');
...
app.post('/api/admin/employees', requireAdmin, async (req, res) => {
    ...
    try {
        ...
        const rows = await query(
            ...
            [name, hashed, parseFloat(hourly_rate) || 15.00, role || 'employee']
        );
        buildPinMap().catch(()=>{});  // refresh cache async
    ...
    try {
        if (pin && pin.length === 4) {
            ...
            await query('UPDATE employees SET name=?,hourly_rate=?,active=?,pin=?,pay_period=? WHERE id=?',
                [name, parseFloat(hourly_rate), active, hashed, pp, req.params.id]);
        } else {
...
app.patch('/api/admin/employees/:id/can-log-hours', requireAdmin, async (req, res) => {
    const val = req.body.can_log_hours ? 1 : 0;
    try {
        await query('UPDATE employees SET can_log_hours=? WHERE id=?', [val, req.params.id]);
        buildPinMap().catch(() => {});
        res.json({ success: true });
    } catch (err) { res.status(500).json({ success: false, error: err.message }); }
});
...
app.get('/api/admin/employee/:id', requireAdmin, async (req, res) => {
    ...
    try {
        const emps = await query('SELECT id,name,role,hourly_rate,pay_period,active,can_log_hours,created_at FROM employees WHERE id=?', [id]);
        ...
        const hours = mins / 60;
        const gross = calcGrossPay(punches, emp.hourly_rate);
...
app.get('/api/report/tax', requireAdmin, async (req, res) => {
    ...
    try {
        ...
        const results = await Promise.all(employees.map(async emp => {
            ...
            const mins = calcMinutes(punches);
            const hours = mins / 60, gross = calcGrossPay(punches, emp.hourly_rate);
            const totalTips = tips.reduce((s, t) => s + (t.total_tips || 0), 0);
            ...
            Object.entries(mPunches).forEach(([m, ps]) => {
                ...
                monthly[m].gross = calcGrossPay(ps, emp.hourly_rate).toFixed(2);
            });
            tips.forEach(t => {
...
app.get('/api/report/csv', requireAdmin, async (req, res) => {
    ...
    try {
        ...
        lines.push('Employee,Hours,Gross Pay,Tips,Total Earnings');
        let gh = 0, gp = 0, gt = 0;
        Object.entries(empMap).forEach(([name, d]) => {
            const mins = calcMinutes(d.punches);
            const h = Math.round(mins / 60 * 10000) / 10000;
            const pay = Math.round(calcGrossPay(d.punches, d.rate) * 100) / 100;
            const tips = Math.round(d.tips * 100) / 100;
            gh += h; gp += pay; gt += tips;
            lines.push(`"${name}",${h.toFixed(4)},${pay.toFixed(2)},${tips.toFixed(2)},${(pay + tips).toFixed(2)}`);
        });
        ...
        lines.push('Employee,Date,Time,Type,Role,Rate');
        punches.forEach(p => {
            const dt = new Date(p.timestamp);
            lines.push(`"${p.employee_name}","${dt.toLocaleDateString()}","${dt.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}","${p.punch_type === 'in' ? 'Clock In' : 'Clock Out'}","${p.work_role||''}","${p.role_rate!=null?p.role_rate:p.hourly_rate}"`);
        });
        lines.push('');
...
app.post('/api/admin/reset-data', requireAdmin, async (req, res) => {
    const { confirm, keep_employees } = req.body;
    if (confirm !== 'RESET') return res.status(400).json({ success: false, message: 'Must confirm with RESET' });
    try {
        await query('DELETE FROM tips');
        await query('DELETE FROM punches');
        await query('DELETE FROM backups');
        if (!keep_employees) {
            await query('DELETE FROM employee_roles');
            await query('DELETE FROM employees');
            await seedEmployees();
            await seedEmployeeRoles();
        }
        res.json({ success: true, message: keep_employees ? 'Punches, tips & backups cleared. Employees kept.' : 'Full reset complete. Employees re-seeded.' });
    } catch (err) { res.status(500).json({ success: false, error: err.message }); }
});
